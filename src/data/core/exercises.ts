// src/data/exercises.ts

// Define the Exercise type
export interface Exercise {
    id: string;
    exercise_name: string;
    category_id: string;
    spring_setup: {
        light_springs: number;
        heavy_springs: number;
    };
    template_tags: string[];
    vimeo_id: string;
    standard_time: string;
    muscle_tags: string[];
    visual_cue: string;
    setup_cues: string[];
    movement_cues: string[];
    breathing_cues: {
        exhale: string;
        inhale: string;
    };
    common_mistakes: string[];
    easier_modification: string;
    harder_progression: string;
}

// Abs exercises (category_id: "c1")
const abs_exercises = {
    e1: {
        id: "e1",
        exercise_name: "ab wheel",
        category_id: "c1",
        spring_setup: {
            light_springs: 1,
            heavy_springs: 0
        },
        template_tags: ["planks"],
        vimeo_id: "",
        standard_time: "1:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: {
            exhale: "",
            inhale: ""
        },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e2: {
        id: "e2",
        exercise_name: "body saw",
        category_id: "c1",
        spring_setup: {
            light_springs: 1,
            heavy_springs: 0
        },
        template_tags: ["planks"],
        vimeo_id: "",
        standard_time: "1:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: {
            exhale: "",
            inhale: ""
        },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e3: {
        id: "e3",
        exercise_name: "cobra",
        category_id: "c1",
        spring_setup: {
            light_springs: 1,
            heavy_springs: 0
        },
        template_tags: ["planks"],
        vimeo_id: "",
        standard_time: "1:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: {
            exhale: "",
            inhale: ""
        },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e4: {
        id: "e4",
        exercise_name: "Plank Crunch",
        category_id: "c1",
        spring_setup: {
            light_springs: 1,
            heavy_springs: 0
        },
        template_tags: ["planks"],
        vimeo_id: "1HJhO1NHnwdCZWfJUVmSSrZNn11YIH24R",
        standard_time: "1:00",
        muscle_tags: ["Rectus Abdominis", "Hip Flexors", "Core Stabilizers"],
        visual_cue: "Keep your back flat like a table as you draw your knees under your hips",
        setup_cues: [
            "Hands on front platform, shoulders stacked over wrists",
            "Feet on carriage in plank position",
            "Core engaged with neutral spine",
            "Body forms straight line from head to heels"
        ],
        movement_cues: [
            "Draw knees underneath hips while maintaining neutral spine",
            "Keep back flat - no spinal flexion or rounding",
            "Extend legs back with control, resist the spring",
            "Maintain shoulder stability and neutral spine throughout"
        ],
        breathing_cues: {
            exhale: "Exhale as you draw knees in",
            inhale: "Inhale as you extend back"
        },
        common_mistakes: [
            "Rounding or flexing the spine during knee draw",
            "Shoulders shifting backward",
            "Losing neutral spine position",
            "Moving too fast without control"
        ],
        easier_modification: "Add a light spring for more support",
        harder_progression: "Slow 4-count tempo, hold at tucked position"
    },
    e5: {
        id: "e5",
        exercise_name: "plank to pike",
        category_id: "c1",
        spring_setup: {
            light_springs: 1,
            heavy_springs: 0
        },
        template_tags: ["planks"],
        vimeo_id: "",
        standard_time: "1:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: {
            exhale: "",
            inhale: ""
        },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e6: {
        id: "e6",
        exercise_name: "high bar plank crunch",
        category_id: "c1",
        spring_setup: {
            light_springs: 1,
            heavy_springs: 0
        },
        template_tags: ["planks"],
        vimeo_id: "",
        standard_time: "1:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: {
            exhale: "",
            inhale: ""
        },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e7: {
        id: "e7",
        exercise_name: "arm assisted cable crunches",
        category_id: "c1",
        spring_setup: {
            light_springs: 2,
            heavy_springs: 0
        },
        template_tags: ["supine_cables_crunches"],
        vimeo_id: "",
        standard_time: "1:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: {
            exhale: "",
            inhale: ""
        },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e8: {
        id: "e8",
        exercise_name: "footstrap crunches",
        category_id: "c1",
        spring_setup: {
            light_springs: 2,
            heavy_springs: 0
        },
        template_tags: ["supine_cables_crunches"],
        vimeo_id: "",
        standard_time: "1:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: {
            exhale: "",
            inhale: ""
        },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e9: {
        id: "e9",
        exercise_name: "leg circles",
        category_id: "c1",
        spring_setup: {
            light_springs: 2,
            heavy_springs: 0
        },
        template_tags: ["supine_cables_crunches"],
        vimeo_id: "",
        standard_time: "1:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: {
            exhale: "",
            inhale: ""
        },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e10: {
        id: "e10",
        exercise_name: "kneeling crunch",
        category_id: "c1",
        spring_setup: {
            light_springs: 2,
            heavy_springs: 0
        },
        template_tags: ["crunches"],
        vimeo_id: "",
        standard_time: "1:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: {
            exhale: "",
            inhale: ""
        },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    }
};

// Obliques exercises (category_id: "c2")
const obliques_exercises = {
    e11: {
        id: "e11",
        exercise_name: "kneeling torso twist",
        category_id: "c2",
        spring_setup: {
            light_springs: 2,
            heavy_springs: 0
        },
        template_tags: ["cables_rotational"],
        vimeo_id: "",
        standard_time: "1:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: {
            exhale: "",
            inhale: ""
        },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e12: {
        id: "e12",
        exercise_name: "standing torso twist",
        category_id: "c2",
        spring_setup: {
            light_springs: 2,
            heavy_springs: 0
        },
        template_tags: ["cables_rotational"],
        vimeo_id: "",
        standard_time: "1:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: {
            exhale: "",
            inhale: ""
        },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e13: {
        id: "e13",
        exercise_name: "split squat torso twist",
        category_id: "c2",
        spring_setup: {
            light_springs: 2,
            heavy_springs: 0
        },
        template_tags: ["cables_rotational"],
        vimeo_id: "",
        standard_time: "1:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: {
            exhale: "",
            inhale: ""
        },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e14: {
        id: "e14",
        exercise_name: "oblique sweeps",
        category_id: "c2",
        spring_setup: {
            light_springs: 2,
            heavy_springs: 0
        },
        template_tags: ["cables_anti_rotational"],
        vimeo_id: "",
        standard_time: "1:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: {
            exhale: "",
            inhale: ""
        },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e15: {
        id: "e15",
        exercise_name: "froggy kicks",
        category_id: "c2",
        spring_setup: {
            light_springs: 2,
            heavy_springs: 0
        },
        template_tags: ["cables_anti_rotational"],
        vimeo_id: "",
        standard_time: "1:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: {
            exhale: "",
            inhale: ""
        },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e16: {
        id: "e16",
        exercise_name: "bird dogs",
        category_id: "c2",
        spring_setup: {
            light_springs: 2,
            heavy_springs: 0
        },
        template_tags: ["cables_anti_rotational"],
        vimeo_id: "",
        standard_time: "1:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: {
            exhale: "",
            inhale: ""
        },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    }
};

// Lower Body LSD exercises (category_id: "c4")
const lower_body_lsd_exercises = {
    e17: {
        id: "e17",
        exercise_name: "platform lunge (front)",
        category_id: "c4",
        spring_setup: { light_springs: 1, heavy_springs: 0 },
        template_tags: ["lunges_light_springs"],
        vimeo_id: "",
        standard_time: "2:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e18: {
        id: "e18",
        exercise_name: "platform lunge (back)",
        category_id: "c4",
        spring_setup: { light_springs: 1, heavy_springs: 0 },
        template_tags: ["lunges_light_springs"],
        vimeo_id: "",
        standard_time: "2:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e19: {
        id: "e19",
        exercise_name: "carriage lunge (front)",
        category_id: "c4",
        spring_setup: { light_springs: 1, heavy_springs: 0 },
        template_tags: ["lunges_light_springs"],
        vimeo_id: "",
        standard_time: "2:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e20: {
        id: "e20",
        exercise_name: "carriage lunge (back)",
        category_id: "c4",
        spring_setup: { light_springs: 1, heavy_springs: 0 },
        template_tags: ["lunges_light_springs"],
        vimeo_id: "",
        standard_time: "2:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e21: {
        id: "e21",
        exercise_name: "floor lunge",
        category_id: "c4",
        spring_setup: { light_springs: 1, heavy_springs: 0 },
        template_tags: ["lunges_light_springs"],
        vimeo_id: "",
        standard_time: "2:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e22: {
        id: "e22",
        exercise_name: "power lunge",
        category_id: "c4",
        spring_setup: { light_springs: 1, heavy_springs: 0 },
        template_tags: ["lunges_light_springs"],
        vimeo_id: "",
        standard_time: "2:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e23: {
        id: "e23",
        exercise_name: "lateral lunge (front)",
        category_id: "c4",
        spring_setup: { light_springs: 1, heavy_springs: 0 },
        template_tags: ["lunges_light_springs"],
        vimeo_id: "",
        standard_time: "2:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e24: {
        id: "e24",
        exercise_name: "lateral lunge (back)",
        category_id: "c4",
        spring_setup: { light_springs: 1, heavy_springs: 0 },
        template_tags: ["lunges_light_springs"],
        vimeo_id: "",
        standard_time: "2:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e25: {
        id: "e25",
        exercise_name: "kneeling inner thighs",
        category_id: "c4",
        spring_setup: { light_springs: 1, heavy_springs: 0 },
        template_tags: ["lunges_light_springs"],
        vimeo_id: "",
        standard_time: "2:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e26: {
        id: "e26",
        exercise_name: "Squats (Heavy)",
        category_id: "c4",
        spring_setup: { light_springs: 1, heavy_springs: 1 },
        template_tags: ["squats_and_deadlifts"],
        vimeo_id: "",
        standard_time: "2:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e27: {
        id: "e27",
        exercise_name: "Squats (Light)",
        category_id: "c4",
        spring_setup: { light_springs: 1, heavy_springs: 0 },
        template_tags: ["squats_and_deadlifts"],
        vimeo_id: "",
        standard_time: "2:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e28: {
        id: "e28",
        exercise_name: "Deadlifts",
        category_id: "c4",
        spring_setup: { light_springs: 1, heavy_springs: 1 },
        template_tags: ["squats_and_deadlifts"],
        vimeo_id: "",
        standard_time: "2:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e29: {
        id: "e29",
        exercise_name: "Pistol Squats",
        category_id: "c4",
        spring_setup: { light_springs: 1, heavy_springs: 0 },
        template_tags: ["squats_and_deadlifts"],
        vimeo_id: "",
        standard_time: "2:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    }
};

// Lower Body Heavy exercises (category_id: "c3")
const lower_body_heavy_exercises = {
    e30: {
        id: "e30",
        exercise_name: "Running Man",
        category_id: "c3",
        spring_setup: { light_springs: 1, heavy_springs: 1 },
        template_tags: ["heavy_pressing"],
        vimeo_id: "",
        standard_time: "2:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e31: {
        id: "e31",
        exercise_name: "Skater Kick",
        category_id: "c3",
        spring_setup: { light_springs: 1, heavy_springs: 1 },
        template_tags: ["heavy_pressing"],
        vimeo_id: "",
        standard_time: "2:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e32: {
        id: "e32",
        exercise_name: "Donkey Kick",
        category_id: "c3",
        spring_setup: { light_springs: 1, heavy_springs: 1 },
        template_tags: ["heavy_pressing"],
        vimeo_id: "",
        standard_time: "2:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e33: {
        id: "e33",
        exercise_name: "Scooter Kick",
        category_id: "c3",
        spring_setup: { light_springs: 1, heavy_springs: 1 },
        template_tags: ["heavy_pressing"],
        vimeo_id: "",
        standard_time: "2:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e34: {
        id: "e34",
        exercise_name: "Well Kick",
        category_id: "c3",
        spring_setup: { light_springs: 1, heavy_springs: 1 },
        template_tags: ["heavy_pressing"],
        vimeo_id: "",
        standard_time: "2:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e35: {
        id: "e35",
        exercise_name: "Standing Side Kicks",
        category_id: "c3",
        spring_setup: { light_springs: 1, heavy_springs: 1 },
        template_tags: ["heavy_pressing"],
        vimeo_id: "",
        standard_time: "2:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e36: {
        id: "e36",
        exercise_name: "Superhero",
        category_id: "c3",
        spring_setup: { light_springs: 1, heavy_springs: 1 },
        template_tags: ["heavy_pressing"],
        vimeo_id: "",
        standard_time: "2:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e37: {
        id: "e37",
        exercise_name: "Bridge Press",
        category_id: "c3",
        spring_setup: { light_springs: 1, heavy_springs: 1 },
        template_tags: ["heavy_pressing"],
        vimeo_id: "",
        standard_time: "2:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e38: {
        id: "e38",
        exercise_name: "Hip Thrusts",
        category_id: "c3",
        spring_setup: { light_springs: 1, heavy_springs: 1 },
        template_tags: ["heavy_pressing"],
        vimeo_id: "",
        standard_time: "2:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e39: {
        id: "e39",
        exercise_name: "Split Squat with Row",
        category_id: "c3",
        spring_setup: { light_springs: 1, heavy_springs: 1 },
        template_tags: ["heavy_pressing"],
        vimeo_id: "",
        standard_time: "2:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    }
};

// Upper Body Exercises (category_id: "c6")
const upper_body_exercises = {
    e40: {
        id: "e40",
        exercise_name: "Chest Press",
        category_id: "c6",
        spring_setup: { light_springs: 0, heavy_springs: 2 },
        template_tags: ["heavy_pushing"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e41: {
        id: "e41",
        exercise_name: "Tricep Press",
        category_id: "c6",
        spring_setup: { light_springs: 0, heavy_springs: 2 },
        template_tags: ["heavy_pushing"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e42: {
        id: "e42",
        exercise_name: "Lat Pull",
        category_id: "c6",
        spring_setup: { light_springs: 0, heavy_springs: 2 },
        template_tags: ["pulling"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e43: {
        id: "e43",
        exercise_name: "Underhand",
        category_id: "c6",
        spring_setup: { light_springs: 0, heavy_springs: 2 },
        template_tags: ["pulling"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e44: {
        id: "e44",
        exercise_name: "Overhand",
        category_id: "c6",
        spring_setup: { light_springs: 0, heavy_springs: 2 },
        template_tags: ["pulling"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e45: {
        id: "e45",
        exercise_name: "Overhead Press",
        category_id: "c6",
        spring_setup: { light_springs: 3, heavy_springs: 0 },
        template_tags: ["long_cables"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e46: {
        id: "e46",
        exercise_name: "Overhead Tricep Extension",
        category_id: "c6",
        spring_setup: { light_springs: 3, heavy_springs: 0 },
        template_tags: ["long_cables"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e47: {
        id: "e47",
        exercise_name: "Front Raise",
        category_id: "c6",
        spring_setup: { light_springs: 3, heavy_springs: 0 },
        template_tags: ["long_cables"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e48: {
        id: "e48",
        exercise_name: "Serve the platter",
        category_id: "c6",
        spring_setup: { light_springs: 3, heavy_springs: 0 },
        template_tags: ["long_cables"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e49: {
        id: "e49",
        exercise_name: "Newspaper",
        category_id: "c6",
        spring_setup: { light_springs: 3, heavy_springs: 0 },
        template_tags: ["long_cables"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e50: {
        id: "e50",
        exercise_name: "Chest Opener",
        category_id: "c6",
        spring_setup: { light_springs: 3, heavy_springs: 0 },
        template_tags: ["long_cables"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e51: {
        id: "e51",
        exercise_name: "Tricep Kickback",
        category_id: "c6",
        spring_setup: { light_springs: 3, heavy_springs: 0 },
        template_tags: ["long_cables"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e52: {
        id: "e52",
        exercise_name: "Reverse fly",
        category_id: "c6",
        spring_setup: { light_springs: 3, heavy_springs: 0 },
        template_tags: ["long_cables"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e53: {
        id: "e53",
        exercise_name: "Tailone face pull",
        category_id: "c6",
        spring_setup: { light_springs: 3, heavy_springs: 0 },
        template_tags: ["long_cables"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e54: {
        id: "e54",
        exercise_name: "Tailbone bicep curl",
        category_id: "c6",
        spring_setup: { light_springs: 3, heavy_springs: 0 },
        template_tags: ["long_cables"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e55: {
        id: "e55",
        exercise_name: "Bench Press",
        category_id: "c6",
        spring_setup: { light_springs: 0, heavy_springs: 2 },
        template_tags: ["short_cables_heavy"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e56: {
        id: "e56",
        exercise_name: "Close Grip Press",
        category_id: "c6",
        spring_setup: { light_springs: 0, heavy_springs: 2 },
        template_tags: ["short_cables_heavy"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e57: {
        id: "e57",
        exercise_name: "Chest Fly",
        category_id: "c6",
        spring_setup: { light_springs: 0, heavy_springs: 2 },
        template_tags: ["short_cables_heavy"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e58: {
        id: "e58",
        exercise_name: "Bent over rows",
        category_id: "c6",
        spring_setup: { light_springs: 0, heavy_springs: 2 },
        template_tags: ["short_cables_heavy"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e59: {
        id: "e59",
        exercise_name: "Squat rows",
        category_id: "c6",
        spring_setup: { light_springs: 0, heavy_springs: 2 },
        template_tags: ["short_cables_heavy"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e60: {
        id: "e60",
        exercise_name: "Tricep Kickback",
        category_id: "c6",
        spring_setup: { light_springs: 2, heavy_springs: 0 },
        template_tags: ["short_cables_light"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e61: {
        id: "e61",
        exercise_name: "Chest Opener",
        category_id: "c6",
        spring_setup: { light_springs: 2, heavy_springs: 0 },
        template_tags: ["short_cables_light"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e62: {
        id: "e62",
        exercise_name: "Lateral Raise",
        category_id: "c6",
        spring_setup: { light_springs: 2, heavy_springs: 0 },
        template_tags: ["short_cables_light"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    }
};

// Cardio Bursts Exercises (category_id: "c7")
const cardio_bursts_exercises = {
    e65: {
        id: "e65",
        exercise_name: "Burpees",
        category_id: "c7",
        spring_setup: { light_springs: 0, heavy_springs: 0 },
        template_tags: ["cardio"],
        vimeo_id: "",
        standard_time: "1:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e66: {
        id: "e66",
        exercise_name: "Mt Climbers",
        category_id: "c7",
        spring_setup: { light_springs: 0, heavy_springs: 0 },
        template_tags: ["cardio"],
        vimeo_id: "",
        standard_time: "1:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e67: {
        id: "e67",
        exercise_name: "Pushups",
        category_id: "c7",
        spring_setup: { light_springs: 0, heavy_springs: 0 },
        template_tags: ["cardio"],
        vimeo_id: "",
        standard_time: "1:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e68: {
        id: "e68",
        exercise_name: "Up/Downs",
        category_id: "c7",
        spring_setup: { light_springs: 0, heavy_springs: 0 },
        template_tags: ["cardio"],
        vimeo_id: "",
        standard_time: "1:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e69: {
        id: "e69",
        exercise_name: "Army Crawls",
        category_id: "c7",
        spring_setup: { light_springs: 0, heavy_springs: 0 },
        template_tags: ["cardio"],
        vimeo_id: "",
        standard_time: "1:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e70: {
        id: "e70",
        exercise_name: "Step Ups",
        category_id: "c7",
        spring_setup: { light_springs: 0, heavy_springs: 0 },
        template_tags: ["cardio"],
        vimeo_id: "",
        standard_time: "1:00",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    }
};

// Lower Body Straps Exercises (category_id: "c5")
const lower_body_straps_exercises = {
    e63: {
        id: "e63",
        exercise_name: "Clamshell Side planks",
        category_id: "c5",
        spring_setup: { light_springs: 2, heavy_springs: 0 },
        template_tags: ["footstraps"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    },
    e64: {
        id: "e64",
        exercise_name: "Side lying legs",
        category_id: "c5",
        spring_setup: { light_springs: 2, heavy_springs: 0 },
        template_tags: ["footstraps"],
        vimeo_id: "",
        standard_time: "1:30",
        muscle_tags: [],
        visual_cue: "",
        setup_cues: [],
        movement_cues: [],
        breathing_cues: { exhale: "", inhale: "" },
        common_mistakes: [],
        easier_modification: "",
        harder_progression: ""
    }
};

// Export all exercise data
export {
    abs_exercises,
    obliques_exercises,
    lower_body_lsd_exercises,
    lower_body_heavy_exercises,
    upper_body_exercises,
    cardio_bursts_exercises,
    lower_body_straps_exercises
};

// Create a flattened array of all exercises for component testing
export const exercisesMock: Exercise[] = [
    ...Object.values(abs_exercises),
    ...Object.values(obliques_exercises),
    ...Object.values(lower_body_lsd_exercises),
    ...Object.values(lower_body_heavy_exercises),
    ...Object.values(upper_body_exercises),
    ...Object.values(cardio_bursts_exercises),
    ...Object.values(lower_body_straps_exercises)
];
